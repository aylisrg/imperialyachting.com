...
function buildPlanCharterPrompt(args: {
  ...
  return [
    ...
    "4. Once a date/time/hours is confirmed, call `check_availability` for the chosen yacht to confirm the slot is free.",
    ...
    "6. Present the quote clearly — base price, extras, bonus hours if applicable, and total — then, only after I explicitly confirm I agree to the quote and the 50% non-refundable deposit, call `create_checkout` and share the deposit checkout link so I can confirm the booking.",
    "",
 ...
 * quote -> checkout, using `check_availability`, `create_quote`, and
 * `create_checkout` (all registered in `server.ts`) by name.
 */
